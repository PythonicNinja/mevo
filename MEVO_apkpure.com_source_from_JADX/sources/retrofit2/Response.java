...
public final class Response<T> {
    ...
    public static <T> Response<T> success(@Nullable T t) {
        return success((Object) t, new Builder().code(200).message(ResponseGoogleRoute.STATUS_OK).protocol(Protocol.HTTP_1_1).request(new Request.Builder().url("http://localhost/").build()).build());
    }

    public static <T> Response<T> success(@Nullable T t, Headers headers) {
        Utils.checkNotNull(headers, "headers == null");
        return success((Object) t, new Builder().code(200).message(ResponseGoogleRoute.STATUS_OK).protocol(Protocol.HTTP_1_1).headers(headers).request(new Request.Builder().url("http://localhost/").build()).build());
    }
