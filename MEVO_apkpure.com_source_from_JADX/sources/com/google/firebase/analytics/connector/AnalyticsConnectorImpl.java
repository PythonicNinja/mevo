...
public class AnalyticsConnectorImpl implements AnalyticsConnector {
    ...
    public AnalyticsConnectorHandle registerAnalyticsConnectorListener(@NonNull final String str, AnalyticsConnectorListener analyticsConnectorListener) {
        ...
        return new AnalyticsConnectorHandle(this) {
            private final /* synthetic */ AnalyticsConnectorImpl zzbqk;

            @KeepForSdk
            public void registerEventNames(Set<String> set) {
                if (this.zzbqk.zzfe(str) && str.equals(AppMeasurement.FIAM_ORIGIN) && set != null && !set.isEmpty()) {
                    ((zza) this.zzbqk.zzbqj.get(str)).registerEventNames(set);
                }
            }

            public void unregister() {
                if (this.zzbqk.zzfe(str)) {
                    AnalyticsConnectorListener zzss = ((zza) this.zzbqk.zzbqj.get(str)).zzss();
                    if (zzss != null) {
                        zzss.onMessageTriggered(0, null);
                    }
                    this.zzbqk.zzbqj.remove(str);
                }
            }

            @KeepForSdk
            public void unregisterEventNames() {
                if (this.zzbqk.zzfe(str) && str.equals(AppMeasurement.FIAM_ORIGIN)) {
                    ((zza) this.zzbqk.zzbqj.get(str)).unregisterEventNames();
                }
            }
        };
    }
