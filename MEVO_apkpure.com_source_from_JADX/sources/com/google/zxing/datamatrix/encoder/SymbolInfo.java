    ...
    public static SymbolInfo lookup(int i, SymbolShapeHint symbolShapeHint, Dimension dimension, Dimension dimension2, boolean z) {
        for (SymbolInfo symbolInfo : symbols) {
            if (symbolShapeHint != SymbolShapeHint.FORCE_SQUARE || !symbolInfo.rectangular) {
                if (symbolShapeHint != SymbolShapeHint.FORCE_RECTANGLE || symbolInfo.rectangular) {
                    if (dimension != null) {
                        if (symbolInfo.getSymbolWidth() < dimension.getWidth()) {
                            continue;
                        } else if (symbolInfo.getSymbolHeight() < dimension.getHeight()) {
                        }
                    }
                    if (dimension2 != null) {
                        if (symbolInfo.getSymbolWidth() > dimension2.getWidth()) {
                            continue;
                        } else if (symbolInfo.getSymbolHeight() > dimension2.getHeight()) {
                        }
                    }
        ...
        dimension.append("Can't find a symbol arrangement that matches the message. Data codewords: ");
        dimension.append(i);
