...
final class MatrixUtil {
    ...
    static void embedTypeInfo(ErrorCorrectionLevel errorCorrectionLevel, int i, ByteMatrix byteMatrix) throws WriterException {
        ...
        for (i = 0; i < bitArray.getSize(); i++) {
            ...
            byteMatrix.set(TYPE_INFO_COORDINATES[i][null], TYPE_INFO_COORDINATES[i][1], z);
            if (i < 8) {
    ...
    static void embedDataBits(BitArray bitArray, int i, ByteMatrix byteMatrix) throws WriterException {
        ...
        if (i2 != bitArray.getSize()) {
            byteMatrix = new StringBuilder();
            byteMatrix.append("Not all bits consumed: ");
            byteMatrix.append(i2);
            byteMatrix.append('/');
            byteMatrix.append(bitArray.getSize());
            throw new WriterException(byteMatrix.toString());
        }
    }
    ...
    private static void maybeEmbedPositionAdjustmentPatterns(Version version, ByteMatrix byteMatrix) {
        if (version.getVersionNumber() >= 2) {
            ...
            int[] iArr = POSITION_ADJUSTMENT_PATTERN_COORDINATE_TABLE[version];
            version = POSITION_ADJUSTMENT_PATTERN_COORDINATE_TABLE[version].length;
