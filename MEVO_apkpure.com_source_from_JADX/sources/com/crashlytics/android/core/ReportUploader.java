    ...
    private class Worker extends BackgroundPriorityRunnable {
        ...
        public void onRun() {
            try {
                ...
                Fabric.getLogger().mo2497e(CrashlyticsCore.TAG, "An unexpected error occurred while attempting to upload crash reports.", e);
            }
...
*/
    ...
    public synchronized void uploadReports(float f, SendCheck sendCheck) {
        if (this.uploadThread != null) {
            Fabric.getLogger().mo2494d(CrashlyticsCore.TAG, "Report upload has already been started.");
            return;
        }
    ...
    List<Report> findReports() {
        Fabric.getLogger().mo2494d(CrashlyticsCore.TAG, "Checking for crash reports...");
        synchronized (this.fileAccessLock) {
        ...
        for (String str2 : hashMap.keySet()) {
            ...
            logger2.mo2494d(str3, stringBuilder2.toString());
            List list = (List) hashMap.get(str2);
            linkedList.add(new InvalidSessionReport(str2, (File[]) list.toArray(new File[list.size()])));
        }
        if (linkedList.isEmpty()) {
            Fabric.getLogger().mo2494d(CrashlyticsCore.TAG, "No reports found.");
        }
        return linkedList;
    }
}
