...
class MessageThreadUtil<T> implements ThreadUtil<T> {
    ...
    public MainThreadCallback<T> getMainThreadProxy(final MainThreadCallback<T> mainThreadCallback) {
        return new MainThreadCallback<T>() {
            ...
            private Runnable mMainThreadRunnable = new C02571();
            ...
            class C02571 implements Runnable {
                ...
                public void run() {
                    SyncQueueItem next = C06041.this.mQueue.next();
                    while (next != null) {
                        switch (next.what) {
                            case 1:
    ...
    public BackgroundCallback<T> getBackgroundProxy(final BackgroundCallback<T> backgroundCallback) {
        return new BackgroundCallback<T>() {
            ...
            private Runnable mBackgroundRunnable = new C02581();
            ...
            class C02581 implements Runnable {
                ...
                public void run() {
                    while (true) {
                        SyncQueueItem next = C06052.this.mQueue.next();
                        if (next != null) {
                            switch (next.what) {
                                case 1:
                                    C06052.this.mQueue.removeMessages(1);
                                    backgroundCallback.refresh(next.arg1);
                                    break;
                                case 2:
                                    C06052.this.mQueue.removeMessages(2);
                                    C06052.this.mQueue.removeMessages(3);
                        ...
                        C06052.this.mBackgroundRunning.set(false);
                        return;
                    }
                }
            }
